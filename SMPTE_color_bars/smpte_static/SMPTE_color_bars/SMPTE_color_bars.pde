...
void draw(){
  ...
  for(int i=0; i < top_bar_amount; i++){
    fill(top_colors[i % top_bar_amount]);
    rect(i * one_seventh_w, 0, one_seventh_w, top_height);
  }
  color mid_color;
  for(int j = 0; j < top_bar_amount; j++){
    ...
    fill(mid_color);
    rect(j * one_seventh_w, top_height, one_seventh_w, mid_height);
  }
  ...
  for (int k=0; k < intial_lower_bar_amount; k++) {
    fill(lower_colors[k]);
    rect(k * lower_bars_w, height - lower_height, lower_bars_w, lower_height);
  }
  ...
  for (int l=0; l < thin_lower_bar_amount; l++) {
    fill(lower_colors[intial_lower_bar_amount + l]);
    rect(intial_lower_bar_amount * lower_bars_w + (l * thin_lower_bar_width), height - lower_height, thin_lower_bar_width, lower_height);
  }
}
